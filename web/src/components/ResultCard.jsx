...
export default function ResultCard({ resource, index, onCopy }) {
  ...
  return (
    <article
      className="group relative bg-white rounded-xl p-4 sm:p-6 border-2 border-gray-100 shadow-sm transition-all duration-200 hover:border-primary-light hover:shadow-md hover:-translate-y-1 focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2"
    >
      {/* Type Badge & Title */}
      <div className="flex items-start gap-3 mb-4 flex-wrap">
        ...
        <h3 className="m-0 text-lg sm:text-xl font-bold text-gray-900 flex-1 min-w-0 tracking-tight leading-snug">
          {resource.title}
        </h3>
      </div>
      ...
      {resource.description && (
        <p className="text-sm text-gray-700 mb-4 leading-relaxed line-clamp-2 sm:line-clamp-none">
          {resource.description}
        </p>
      )}

      {/* URL Section with Copy Button */}
      <div className="flex items-center gap-2 p-3 bg-gradient-to-br from-slate-50 to-slate-100 rounded-lg border border-gray-200 transition-all hover:from-slate-100 hover:to-slate-200 hover:border-primary-light">
        <a
          href={resource.url}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 min-w-0 text-xs sm:text-sm text-primary font-medium no-underline break-all leading-relaxed transition-colors hover:text-primary-dark hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:rounded"
        ...
        <button
          onClick={handleCopy}
          className="flex-shrink-0 px-3 py-1.5 bg-white border-2 border-gray-300 rounded-md text-sm font-semibold transition-all hover:bg-primary hover:text-white hover:border-primary hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
